...
export function DataTable<TData extends Record<string, unknown>, TValue>({
  ...
  emptyMessage?: string;
  onRowClick?: (row: TData) => void; // Add this type
}) {
  ...
  let tableName = 'table-export.csv';
  const pathname = usePathname();
  if (columnFileName) {
    tableName =
      columnFileName
        .replace(/Columns?$/, '') // remove 'Columns' or 'Column'
        .replace(/([a-z])([A-Z])/g, '$1-$2') // camelCase to kebab-case
        .replace(/_/g, '-')
        .toLowerCase() + '.csv';
  } else if (pathname) {
    // fallback to previous logic
    const match = pathname.match(/([a-zA-Z-]+)(?:\/|$)/g);
    if (match && match.length > 0) {
      const last = match[match.length - 1].replace(/\/$/, '');
      if (last) tableName = `${last}.csv`;
    }
  }

  // If the columns prop has a displayName or id, try to use that
  // Otherwise, fallback to the above
    ...
    <div>
      <div className="flex flex-wrap items-center justify-between gap-4 pb-4">
        ...
        <div className="flex items-center gap-3">
          {/* Export Dropdown - always show if there is data */}
          {data.length > 0 && (
            <ExportButton
              onClick={() => {}}
              ...
              table.getRowModel().rows.map((row) => (
                ...
                >
                  {row.getVisibleCells().map((cell) => (
                    <TableCell className="px-3 py-3.5" key={cell.id}>
                      {flexRender(
